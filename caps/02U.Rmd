...
Observando los primeros 15 registros —Tabla \@ref(tab:ephind319s)— de la base podemos dar cuenta que la misma se ha cargado correctamente y explorar las distintas variables que contiene. 
...
En este ejemplo —Tabla \@ref(tab:ephhog414)— utilizaremos la base de hogares del cuarto trimestre de 2014 ([Hogar_t414](https://www.indec.gob.ar/ftp/cuadros/menusuperior/eph/t414_sav.rar){target="_blank"}) y la descargaremos en formato ```.sav```.
...
En la Tabla \@ref(tab:ephind74) podemos ver los primeros 15 registros de la base:
...
Teniendo en cuenta los primeros 15 casos —Tabla \@ref(tab:eph319s)—, podemos observar que el número de variables asciende a `r ncol(eph_319)`:
